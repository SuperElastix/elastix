...
AdaptiveStochasticVarianceReducedGradient<TElastix>
...
template <class TElastix>
void AdaptiveStochasticVarianceReducedGradient<TElastix>
::BeforeEachResolution( void )
{
  ...
  this->GetConfiguration()->ReadParameter( maximumNumberOfInnerLoopIterations,
    "MaximumNumberOfInnerLoopIterations", this->GetComponentLabel(), level, 0 );
    ...
    * function. */
    ...
    for( unsigned int m = 0; m < M; ++m )
    {
     ...
     samplerVec[ m ] =
      ...
      randomSamplerVec[ m ]->SetInputImageRegion( samplerVec[ m ]->
        GetInputImageRegion() );
...
//       subRandomSamplerVec[ m ]->SetMask( randomSamplerVec[m]->GetMask() );
      ...
      subRandomSamplerVec[ m ]->SetInputImageRegion( samplerVec[ m ]->
        GetInputImageRegion() );
...
//      subRandomSamplerVec[ m ]-> SetRandomSampleContainer( randomSampleContainer[m], itp );
      ...
      this->GetElastix()->GetElxMetricBase( m )
        ->SetAdvancedMetricImageSampler( subRandomSamplerVec[ m ]);
    ...
    for ( unsigned i = 0; i < this->m_NumberOfInnerIterations; i++ )
    {
      ...
      this->GetConfiguration()->ReadParameter( this->m_UseNoiseFactor,
        "UseNoiseFactor", this->GetComponentLabel(), 0, 0 );

      if ( this->m_UseNoiseFactor )
      {
//         for( unsigned int j = 0; j < spaceDimension; ++j )
//         {
//           this->m_Gradient[j] = this->m_NoiseFactor * (localCurrentGradient[j] - localPreviousGradient[j]) + this->m_PreviousGradient[j];
//         }
        for( unsigned int j = 0; j < spaceDimension; ++j )
        {
          this->m_Gradient[j] = this->m_NoiseFactor * (localCurrentGradient[j] - localPreviousGradient[j]) + this->m_MeanGradient[j];
        }
      }
      else
      {
...
::AutomaticParameterEstimationOriginal( void )
{
  ...
  const double omega = vnl_math::max( 1e-14,
  ...
  if( this->GetNewSamplesEveryIteration() )
  {
    for( unsigned int m = 0; m < M; ++m )
    {
      ...
      if( randomSamplerVec[m].IsNotNull() )
      {
        ...
        this->GetConfiguration()->ReadParameter( sampleGradientMethod,
          "SampleGradientMethod", this->GetComponentLabel(), 0, 0 );
