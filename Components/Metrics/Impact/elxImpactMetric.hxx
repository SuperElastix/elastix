    ...
    try
    {
      if (mode == "Static")
      {
        modelsConfiguration.emplace_back(modelsPathVec[i],
                                         ...
                                         GetVectorFromString<int64_t>(modelsDimensionVec[i], patchSizeVec[i], 5, '*'),
                                         GetVectorFromString<float>(imageDimension, voxelSizeVec[i], 1.5, '*'),
                                         GetVectorFromString<bool>(layersMaskVec[i], true, '\0'),
                                         true);
      }
      else
      {
        modelsConfiguration.emplace_back(modelsPathVec[i],
                                         ...
                                         numberOfChannelsVec[i],
                                         GetVectorFromString<int64_t>(modelsDimensionVec[i], patchSizeVec[i], 5, '*'),
                                         GetVectorFromString<float>(modelsDimensionVec[i], voxelSizeVec[i], 1.5, '*'),
                                         GetVectorFromString<bool>(layersMaskVec[i], true, '\0'),
                                         false);
      }
    }
