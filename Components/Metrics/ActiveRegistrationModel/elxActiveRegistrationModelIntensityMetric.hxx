 ...
 */
  ...
  ::BeforeAllBase( void )
  {
    ...
    if( this->m_ImageDirectories.size() > 0 )
    {
      ...
      if( this->GetNoiseVariance().size() != this->m_ImageDirectories.size() &&
          this->GetNoiseVariance().size() > 1 )
      {
        itkExceptionMacro( << "The number of entries for NoiseVariance must either be zero (a default value of " << std::endl
                             << "0.0 is used corresponding to non-probabilistic PCA), one (the same value will " << std::endl
                             << "be used for all models), or equal to the number of models specified by \"BuildIntensityModel\".");
      }

      // Number of principal components to keep
      this->SetNumberOfPrincipalComponents( ReadNumberOfPrincipalComponents( "NumberOfPrincipalComponents" ) );

      if( this->GetNumberOfPrincipalComponents().size() != this->m_ImageDirectories.size() &&
          this->GetNumberOfPrincipalComponents().size() > 1 )
      {
        itkExceptionMacro( << "The number of entries for NumberOfPrincipalComponents must either be zero (all " << std::endl
                             << "principal components will be used), one (the same value will be used for all models), " << std::endl
                             << "or equal to the number of models specified by \"BuildIntensityModel\".");
      }
    }
...
::BeforeRegistration( void )
{
  ...
  if( this->m_LoadIntensityModelFileNames.size() > 0 )
  {
    ...
    for( unsigned int modelNumber = 0; modelNumber < this->m_LoadIntensityModelFileNames.size(); ++modelNumber )
    {
      // Load model
      try
      {
        RepresenterPointer representer = RepresenterType::New();
        StatisticalModelPointer statisticalModel = StatisticalModelType::New();
        itk::StatismoIO<StatisticalModelImageType>::LoadStatisticalModel(representer.GetPointer(), this->m_LoadIntensityModelFileNames[ modelNumber ] );
        statisticalModelContainer->SetElement( modelNumber, dynamic_cast< StatisticalModelType * >( statisticalModel.GetPointer() ) );
      ...
      elxout << "  Loaded model " << this->m_LoadIntensityModelFileNames[ modelNumber ].c_str() << "." << std::endl;
  ...
  if( this->m_ImageDirectories.size() )
  {
    ...
    for( unsigned int modelNumber = 0; modelNumber < this->m_ImageDirectories.size(); ++modelNumber )
    {
      // Load data
      DataManagerPointer dataManager;
      try
      {
        dataManager = this->LoadImagesFromDirectory( this->m_ImageDirectories[ modelNumber ], this->m_ReferenceFilenames[ modelNumber ] );
      }
      catch( statismo::StatisticalModelException &e )
      {
        itkExceptionMacro( "Error loading samples in " << this->m_ImageDirectories[ modelNumber ] <<": " << e.what() );
      }
      ...
      try
      {
        ...
        statisticalModelContainer->SetElement( this->m_LoadIntensityModelFileNames.size()+modelNumber, dynamic_cast< StatisticalModelType * >( statisticalModel.GetPointer() ) );
        statisticalModelOrthonormalPCABasisMatrixContainer->SetElement( modelNumber, statisticalModel->GetOrthonormalPCABasisMatrix() );

        elxout << "  Built statistical intensity model. Number of principal components: "
        <<  statisticalModelContainer->GetElement( this->m_LoadIntensityModelFileNames.size()+modelNumber )->GetNumberOfPrincipalComponents()
        << "." << std::endl;

        // Pick out first principal components
        if( this->GetNumberOfPrincipalComponents()[ modelNumber ] > 0.0 && this->GetNumberOfPrincipalComponents()[ modelNumber ] < statisticalModel->GetNumberOfPrincipalComponents() )
        {
          ReducedVarianceModelBuilderPointer reducedVarianceModelBuilder = ReducedVarianceModelBuilderType::New();
          statisticalModel = reducedVarianceModelBuilder->BuildNewModelWithLeadingComponents( statisticalModel, ( unsigned int ) ( this->GetNumberOfPrincipalComponents()[ modelNumber ] ) );
          statisticalModelContainer->SetElement( this->m_LoadIntensityModelFileNames.size()+modelNumber, dynamic_cast< StatisticalModelType * >( statisticalModel.GetPointer() ) );
          statisticalModelOrthonormalPCABasisMatrixContainer->SetElement( modelNumber, statisticalModel->GetOrthonormalPCABasisMatrix() );

          elxout << "  Number of principal components retained: " << statisticalModelContainer->GetElement( this->m_LoadIntensityModelFileNames.size()+modelNumber )->GetNumberOfPrincipalComponents()
          << "." << std::endl;
        }

        if( this->m_SaveIntensityModelFileNames.size() > 0 ) {
          elxout << "Saving intensity model " << modelNumber << " to " << this->m_SaveIntensityModelFileNames[ modelNumber ] << ". " << std::endl;
          itk::StatismoIO<StatisticalModelImageType>::SaveStatisticalModel(statisticalModel, this->m_SaveIntensityModelFileNames[ modelNumber ]);
        }
      }
      catch( statismo::StatisticalModelException &e )
      {
...
::BeforeEachResolution( void )
{
  ...
  if( level >= this->GetStatisticalModelContainer()->size() )
  {
    itkExceptionMacro( "The number of resolutions is greater than the number of intensity models." )
  }

  if( this->GetStatisticalModelContainer()->size() > 1u )
  {
    this->SetLevel( level );
  }
  else
 ...
 */
  ...
  template< class TElastix >
  typename ActiveRegistrationModelIntensityMetric< TElastix >::StatisticalModelParameterVectorType
  ActiveRegistrationModelIntensityMetric< TElastix >
  ::ReadNoiseVariance( std::string parameter )
  {
    ...
    StatisticalModelParameterVectorType noiseVarianceVector = StatisticalModelParameterVectorType( this->m_ImageDirectories.size(), 0.0 );
...
typename ActiveRegistrationModelIntensityMetric< TElastix >::StatisticalModelParameterVectorType
ActiveRegistrationModelIntensityMetric< TElastix >
::ReadNumberOfPrincipalComponents( std::string parameter )
{
  std::ostringstream key( parameter, std::ios_base::ate );
  key << this->GetMetricNumber();

  StatisticalModelParameterVectorType numberOfPrincipalComponentsVector = StatisticalModelParameterVectorType( this->m_ImageDirectories.size(), 0.0 );
  unsigned int n = this->GetConfiguration()->CountNumberOfParameterEntries( key.str() );

  if( n == 0 )
  {
    elxout << "WARNING: NumberOfPrincipalComponents not specified for " << this->GetComponentLabel() << ":" << this->elxGetClassName() << "." << std::endl
    << "  A default value of 1 will be used." << std::endl;

    return numberOfPrincipalComponentsVector;
  }
  ...
  for(unsigned int i = 0; i < this->GetConfiguration()->CountNumberOfParameterEntries( key.str() ); ++i)
  {
    ...
    char *e;
    errno = 0;
    double numberOfPrincipalComponents = std::strtod( value.c_str(), &e );

    if ( *e != '\0' || // error, we didn't consume the entire string
         errno != 0 )  // error, overflow or underflow
    {
      itkExceptionMacro( << "Invalid number format for NumberOfPrincipalComponents entry " << i << "." );
    }

    if( numberOfPrincipalComponents < 0.0 )
    {
      itkExceptionMacro( << "TotalVariance entries must be greater than zero but entry number " << i << " is " << numberOfPrincipalComponents << ". Please correct your parameter file." );
    }

    elxout << "  " << key.str() << ": " << numberOfPrincipalComponents << std::endl;
    numberOfPrincipalComponentsVector[ i ] = numberOfPrincipalComponents;
  }

  if( n == 1 && numberOfPrincipalComponentsVector.size() > 1 )
  {
    // Need to fill the rest of the elements
    numberOfPrincipalComponentsVector.fill( numberOfPrincipalComponentsVector[ 0 ] );
  }
...
template< class TElastix >
typename ActiveRegistrationModelIntensityMetric< TElastix >::DataManagerType::Pointer
ActiveRegistrationModelIntensityMetric< TElastix >
::LoadImagesFromDirectory(
  ...
  StatisticalModelImagePointer& image )
{
  ...
  {
    elxout << "skipping " << imageFilename << "(not a valid image file or does not exist)." << std::endl;
    return false;
  }
