...
template <class TElastix>
void
MultiBSplineTransformWithNormal<TElastix>::WriteToFile(const ParametersType & param) const
{
  ...
  xout["transpar"] << "(BSplineTransformSplineOrder " << this->m_SplineOrder << ")" << std::endl;

  /** Write the label map path of this transform. */
  xout["transpar"] << "(MultiBSplineTransformWithNormalLabels \""
                   << itksys::SystemTools::CollapseFullPath(this->m_LabelsPath.c_str()) << "\" )" << std::endl;
