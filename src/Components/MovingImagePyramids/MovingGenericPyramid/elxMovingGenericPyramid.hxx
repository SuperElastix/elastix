...
::SetMovingSchedule( void )
{
  ...
  for( unsigned int i = 0; i < numberOfResolutions; i++ )
  {
    for( unsigned int j = 0; j < MovingImageDimension; j++ )
    {
      ...
      ijfound |= this->m_Configuration->ReadParameter( rescaleSchedule[ i ][ j ],
        "MovingImagePyramidRescaleSchedule", entrynr, false );
      ijfound |= this->m_Configuration->ReadParameter( rescaleSchedule[ i ][ j ],
  ...
  for( unsigned int i = 0; i < numberOfResolutions; i++ )
  {
    for( unsigned int j = 0; j < MovingImageDimension; j++ )
    {
      ...
      ijfound |= this->m_Configuration->ReadParameter( smoothingSchedule[ i ][ j ],
        "MovingImagePyramidSmoothingSchedule", entrynr, false );
  ...
  this->m_Configuration->ReadParameter( computeThisResolution,
    "ComputePyramidImagesPerResolution", 0, false );
