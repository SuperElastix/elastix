...
namespace elastix
{
   ...
   *
   * NB: in order to use this class for registration, the -ipp command line argument
   * is mandatory! It is used to place the fixed image landmarks.
   *
   * The parameters used in this class are:
   * \parameter Transform: Select this transform as follows:\n
   *    <tt>(%Transform "SplineKernelTransform")</tt>
   * \parameter SplineKernelType: Select the deformation model, which must be one of
   ...
   * \parameter SplineRelaxationFactor: make the spline interpolating or approximating. 
   * A value of 0.0 gives an interpolating transform. Higher values result in approximating
   * splines.\n
   *   example: <tt>(SplineRelaxationFactor 0.01 )</tt>\n
   * Default: 0.0. You cannot specify this parameter for each resolution differently.
   * \parameter SplinePoissonRatio: Set the poisson ratio for the ElasticBodySpline 
   * and the ElastixBodyReciprocalSpline. For other SplineKernelTypes this parameters is ignored.\n
   *   example: <tt>(SplinePoissonRatio 0.3 )</tt>\n
   * Default: 0.3. You cannot specify this parameter for each resolution differently.\n
   * Valid values are withing -1.0 and 0.5. 0.5 means incompressible. Negative values are a bit odd,
   * but possible. See Wikipedia on PoissonRatio.
   * 
   * \commandlinearg -ipp: a file specifying a set of points that will serve as fixed image landmarks.\n
   *   example: <tt>-ipp fixedImagePoints.txt</tt> \n
   *   The fixedImagePoints.txt file should be structured: first line should be "index" or
   ...
   *   third and following lines give the indices or points. The same structure thus as
   *   used for transformix.\n
   *
   ...
   * splines.\n
   *   example: <tt>(SplineRelaxationFactor 0.01 )</tt>\n   *
   * \transformparameter SplinePoissonRatio: Set the poisson ratio for the ElasticBodySpline 
   * and the ElastixBodyReciprocalSpline. For other SplineKernelTypes this parameters is ignored.\n
   *   example: <tt>(SplinePoissonRatio 0.3 )</tt>\n   
   * Valid values are withing -1.0 and 0.5. 0.5 means incompressible. Negative values are a bit odd,
   * but possible. See Wikipedia on PoissonRatio.
   * \transformparameter FixedImageLandmarks: The landmark positions in the fixed image, in world coordinates.
   * Positions written as x1 y1 [z1] x2 y2 [z2] etc.\n
  ...
  protected:
    ...
    /** Create an instance of a kernel transform. Returns false
     * if the kernelType is unknown */
