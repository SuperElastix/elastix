...
void EulerStackTransform<TElastix>::BeforeRegistration( void )
{
  ...
  this->m_Registration->GetAsITKBaseType()->SetInitialTransformParameters( dummyInitialParameters );
    ...
    for ( unsigned int i = 0; i < ReducedSpaceDimension; i++ )
    {
        ...
        const bool foundP = this->m_Configuration->ReadParameter(centerOfRotationPoint[ i ], "CenterOfRotationPoint", i, false );
        if ( !foundP )
        {
    ...
    if( automaticStackTransformInitialization )
    {
        ...
        transformInitializer->SetTransform( this->m_EulerStackTransform );
        

        transformInitializer->InitializeTransform(centerGivenAsIndex,centerGivenAsPoint,centerOfRotationIndex,centerOfRotationPoint);
