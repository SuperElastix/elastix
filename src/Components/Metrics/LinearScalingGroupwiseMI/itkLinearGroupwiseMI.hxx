...
namespace itk
{
    ...
    {
        std::ofstream output;
        output.open("/Users/mp/Work/Data/SGR/Data/Results.txt",std::ios::app);
        const unsigned int lastDim     = this->GetFixedImage()->GetImageDimension() - 1;
        const unsigned int lastDimSize = this->GetFixedImage()->GetLargestPossibleRegion().GetSize( lastDim );
        ...
        for(unsigned int s = 0; s < this->m_NumSamplesLastDimension; s++)
        {
            ...
            while( fixedPDFit != fixedPDFend )
            {
                ...
                while( movingPDFit != movingPDFend )
                {
                    ...
                    {
                        fixed+=jointPDFValue * vcl_log(fixedImagePDFValue);
                        moving+=jointPDFValue * vcl_log(movingImagePDFValue);
                        joint+=jointPDFValue * vcl_log(jointPDFValue);
        ...
        output << fixed << " " << moving << " " << joint << " " << fixed + moving - joint << std::endl;
        minimize = false;
    }
    ...
    ::InitializeThreadingParameters( void ) const
    {
        ...
        for( ThreadIdType i = 0; i < this->m_NumberOfThreads; ++i )
        {
            ...
            this->m_LinearGroupwiseMIGetValueAndDerivativePerThreadVariables[ i ].st_NumberOfPixelsCountedVector.assign(lastDimSize, NumericTraits< SizeValueType >::Zero);
