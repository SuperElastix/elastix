#ifndef _itkLinearGroupwiseJE_HXX__
#define _itkLinearGroupwiseJE_HXX__

#include "itkLinearGroupwiseJE.h"
...
namespace itk
{
    ...
    LinearGroupwiseJE< TFixedImage, TMovingImage >
    ::LinearGroupwiseJE()
    {
        this->m_UseDerivative = true;
        this->SetUseImageSampler( true );
        
        this->m_LinearGroupwiseJEThreaderParameters.m_Metric = this;
        
        this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariables     = NULL;
        this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariablesSize = 0;
    ...
    ::~LinearGroupwiseJE()
    {
        delete[] this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariables;
        ...
        for(unsigned int s = 0; s < this->m_NumSamplesLastDimension; s++)
        {
            ...
            while( fixedPDFit != fixedPDFend )
            {
                ...
                while( movingPDFit != movingPDFend )
                {
                    ...
                    {
                        const double pRatio      = vcl_log( jointPDFValue / ( fixedImagePDFValue ) );
            ...
            while( fixedPDFit != fixedPDFend )
            {
                ...
                while( movingPDFit != movingPDFend )
                {
                    ...
                    if( jointPDFValue > 1e-16 && fixedImagePDFValue > 1e-16 )
                    {
                        derivit = derivbegin;
                        const double pRatio      = vcl_log( jointPDFValue / ( fixedImagePDFValue) );
                        const double pRatio_e      = vcl_log( ( jointPDFValue ) / ( fixedImagePDFValue ) );
    ...
    ::ComputePDFsThreaderCallback( void * arg )
    {
        ...
        LinearGroupwiseJEMultiThreaderParameterType * temp
        = static_cast< LinearGroupwiseJEMultiThreaderParameterType * >( infoStruct->UserData );
    ...
    ::LaunchComputePDFsThreaderCallback( void ) const
    {
        ...
        local_threader->SetSingleMethod( this->ComputePDFsThreaderCallback, const_cast< void * >( static_cast< const void * >( &this->m_LinearGroupwiseJEThreaderParameters ) ) );
    ...
    ::InitializeThreadingParameters( void ) const
    {
        ...
        if( this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariablesSize != this->m_NumberOfThreads )
        {
            delete[] this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariables;
            this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariables = new AlignedLinearGroupwiseJEGetValueAndDerivativePerThreadStruct[this->m_NumberOfThreads ];
            this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariablesSize = this->m_NumberOfThreads;
        }
        
        for( ThreadIdType i = 0; i < this->m_NumberOfThreads; ++i )
        {
            this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariables[ i ].st_NumberOfPixelsCounted = NumericTraits< SizeValueType >::Zero;
            this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariables[ i ].st_NumberOfPixelsCountedVector.assign(lastDimSize, NumericTraits< SizeValueType >::Zero);
            this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariables[ i ].st_JointPDFs.resize(lastDimSize);
            this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariables[ i ].st_Derivatives.resize(lastDimSize);

            for(unsigned int d = 0; d < lastDimSize; d++)
            {
                JointPDFPointer & jointPDF = this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariables[ i ].st_JointPDFs[ d ];
                ...
                if (this->m_UseExplicitPDFDerivatives)
                {
                    this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariables[ i ].st_Derivatives.clear();
                }
    ...
    ::AfterThreadedComputePDFs( void ) const
    {
        ...
        for( ThreadIdType i = 0; i < this->m_NumberOfThreads; ++i )
        {
            this->m_NumberOfPixelsCounted += this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariables[ i ].st_NumberOfPixelsCounted;

            for(unsigned int s = 0; s < this->m_NumSamplesLastDimension; s++)
            {
                this->m_NumberOfPixelsCountedVector[this->m_RandomList[s]] += this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariables[ i ].st_NumberOfPixelsCountedVector[this->m_RandomList[s]];
            }
        ...
        for(unsigned int s = 0; s < this->m_NumSamplesLastDimension; s++)
        {
            ...
            for( ThreadIdType i = 0; i < this->m_NumberOfThreads; ++i )
            {
                itT[ i ] = JointPDFIteratorType(this-> m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariables[ i ].st_JointPDFs[this->m_RandomList[s]], this->m_JointPDFs[this->m_RandomList[s]]->GetBufferedRegion() );
            }
    ...
    ::ThreadedComputePDFs( ThreadIdType threadId )
    {
        ...
        std::vector<JointPDFPointer> & jointPDFs = this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariables[ threadId ].st_JointPDFs;
        std::vector<SizeValueType> & NumberOfPixelsCountedVector = this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariables[ threadId ].st_NumberOfPixelsCountedVector;
        SizeValueType & NumberOfPixelsCounted = this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariables[ threadId ].st_NumberOfPixelsCounted;
    ...
    ::ComputeValueAndPRatioArray( double & MI, unsigned int n ) const
    {
        ...
        while( fixedPDFit != fixedPDFend )
        {
            ...
            while( movingPDFit != movingPDFend )
            {
                ...
                {
                    const PDFValueType pRatio = vcl_log( jointPDFValue / ( fixedPDFValue ) );
                    const PDFValueType pRatio_e = vcl_log( ( jointPDFValue ) / ( fixedPDFValue )  );
    ...
    ::LaunchComputeDerivativeLowMemoryThreaderCallback( void ) const
    {
        typename ThreaderType::Pointer local_threader = ThreaderType::New();
        local_threader->SetNumberOfThreads( this->m_NumberOfThreads );
        local_threader->SetSingleMethod( this->ComputeDerivativeLowMemoryThreaderCallback, const_cast< void * >( static_cast< const void * >( &this->m_LinearGroupwiseJEThreaderParameters ) ) );
    ...
    ::AfterThreadedComputeDerivativeLowMemory( std::vector<DerivativeType> & derivatives ) const
    {
        for( ThreadIdType i = 0; i < this->m_NumberOfThreads; ++i )
        {
            std::vector<DerivativeType> & derivative = this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariables[ i ].st_Derivatives;
    ...
    ::ThreadedComputeDerivativeLowMemory( ThreadIdType threadId )
    {
        ...
        std::vector<DerivativeType> & derivatives = this->m_LinearGroupwiseJEGetValueAndDerivativePerThreadVariables[ threadId ].st_Derivatives;
...
#endif // end #ifndef _itkLinearGroupwiseJE_HXX__
