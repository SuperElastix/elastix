#ifndef _itkAverageConditionalMI_HXX__
#define _itkAverageConditionalMI_HXX__

#include "itkAverageConditionalMI.h"
...
namespace itk
{
    ...
    AverageConditionalMI< TFixedImage, TMovingImage >
    ::AverageConditionalMI()
    {
        this->m_UseDerivative = true;
        this->SetUseImageSampler( true );
        
        this->m_AverageConditionalMIThreaderParameters.m_Metric = this;
        
        this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariables     = NULL;
        this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariablesSize = 0;
    ...
    ::~AverageConditionalMI()
    {
        delete[] this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariables;
        ...
        for(unsigned int s = 0; s < this->m_NumSamplesLastDimension; s++)
        {
            ...
            while( fixedPDFit != fixedPDFend )
            {
                ...
                while( movingPDFit != movingPDFend )
                {
                    ...
                    {
                        const double pRatio      = vcl_log( jointPDFValue / ( std::pow(fixedImagePDFValue,this->m_NumSamplesLastDimension) ) );
            ...
            while( fixedPDFit != fixedPDFend )
            {
                ...
                while( movingPDFit != movingPDFend )
                {
                    ...
                    if( jointPDFValue > 1e-16 && fixedImagePDFValue > 1e-16 )
                    {
                        derivit = derivbegin;
                        const double pRatio      = vcl_log( jointPDFValue / ( std::pow(fixedImagePDFValue,this->m_NumSamplesLastDimension)) );
                        const double pRatio_e      = vcl_log( ( jointPDFValue ) / ( std::pow(fixedImagePDFValue,this->m_NumSamplesLastDimension)) );
                        const double pRatio_e_Alpha = pRatio_e / static_cast<double>(this->m_NumberOfPixelsCountedVector[this->m_RandomList[s]]);
                        MI = MI + jointPDFValue * pRatio;
                        while( derivit != derivend )
                        {
                            ( *derivit ) += jointPDFDerivativesit.Get() * pRatio_e_Alpha;
                            ++derivit;
    ...
    ::ComputePDFsThreaderCallback( void * arg )
    {
        ...
        AverageConditionalMIMultiThreaderParameterType * temp
        = static_cast< AverageConditionalMIMultiThreaderParameterType * >( infoStruct->UserData );
    ...
    ::LaunchComputePDFsThreaderCallback( void ) const
    {
        ...
        local_threader->SetSingleMethod( this->ComputePDFsThreaderCallback, const_cast< void * >( static_cast< const void * >( &this->m_AverageConditionalMIThreaderParameters ) ) );
    ...
    ::InitializeThreadingParameters( void ) const
    {
        ...
        if( this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariablesSize != this->m_NumberOfThreads )
        {
            delete[] this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariables;
            this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariables = new AlignedAverageConditionalMIGetValueAndDerivativePerThreadStruct[this->m_NumberOfThreads ];
            this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariablesSize = this->m_NumberOfThreads;
        }
        
        for( ThreadIdType i = 0; i < this->m_NumberOfThreads; ++i )
        {
            this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariables[ i ].st_NumberOfPixelsCounted = NumericTraits< SizeValueType >::Zero;
            this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariables[ i ].st_NumberOfPixelsCountedVector.assign(lastDimSize, NumericTraits< SizeValueType >::Zero);
            this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariables[ i ].st_JointPDFs.resize(lastDimSize);
            this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariables[ i ].st_Derivatives.resize(lastDimSize);

            for(unsigned int d = 0; d < lastDimSize; d++)
            {
                JointPDFPointer & jointPDF = this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariables[ i ].st_JointPDFs[ d ];
                ...
                if (this->m_UseExplicitPDFDerivatives)
                {
                    this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariables[ i ].st_Derivatives.clear();
                }
    ...
    ::AfterThreadedComputePDFs( void ) const
    {
        ...
        for( ThreadIdType i = 0; i < this->m_NumberOfThreads; ++i )
        {
            this->m_NumberOfPixelsCounted += this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariables[ i ].st_NumberOfPixelsCounted;

            for(unsigned int s = 0; s < this->m_NumSamplesLastDimension; s++)
            {
                this->m_NumberOfPixelsCountedVector[this->m_RandomList[s]] += this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariables[ i ].st_NumberOfPixelsCountedVector[this->m_RandomList[s]];
            }
        ...
        for(unsigned int s = 0; s < this->m_NumSamplesLastDimension; s++)
        {
            ...
            for( ThreadIdType i = 0; i < this->m_NumberOfThreads; ++i )
            {
                itT[ i ] = JointPDFIteratorType(this-> m_AverageConditionalMIGetValueAndDerivativePerThreadVariables[ i ].st_JointPDFs[this->m_RandomList[s]], this->m_JointPDFs[this->m_RandomList[s]]->GetBufferedRegion() );
            }
    ...
    ::ThreadedComputePDFs( ThreadIdType threadId )
    {
        ...
        std::vector<JointPDFPointer> & jointPDFs = this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariables[ threadId ].st_JointPDFs;
        std::vector<SizeValueType> & NumberOfPixelsCountedVector = this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariables[ threadId ].st_NumberOfPixelsCountedVector;
        SizeValueType & NumberOfPixelsCounted = this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariables[ threadId ].st_NumberOfPixelsCounted;
    ...
    ::ComputeValueAndPRatioArray( double & MI, unsigned int n ) const
    {
        ...
        while( fixedPDFit != fixedPDFend )
        {
            ...
            while( movingPDFit != movingPDFend )
            {
                ...
                if( jointPDFValue > 1e-16 && fixedPDFValue > 1e-16 )
                {
                    const PDFValueType pRatio = vcl_log( jointPDFValue / ( std::pow(fixedPDFValue,this->m_NumSamplesLastDimension)) );
                    const PDFValueType pRatio_e = vcl_log( ( jointPDFValue ) / ( std::pow(fixedPDFValue,this->m_NumSamplesLastDimension))  );
                    
                    (*(this->m_PRatioArray[this->m_RandomList[n]]))[ fixedIndex ][ movingIndex ] = static_cast< PRatioType >(pRatio_e / this->m_NumberOfPixelsCountedVector[this->m_RandomList[n]]);
    ...
    ::LaunchComputeDerivativeLowMemoryThreaderCallback( void ) const
    {
        typename ThreaderType::Pointer local_threader = ThreaderType::New();
        local_threader->SetNumberOfThreads( this->m_NumberOfThreads );
        local_threader->SetSingleMethod( this->ComputeDerivativeLowMemoryThreaderCallback, const_cast< void * >( static_cast< const void * >( &this->m_AverageConditionalMIThreaderParameters ) ) );
    ...
    ::AfterThreadedComputeDerivativeLowMemory( std::vector<DerivativeType> & derivatives ) const
    {
        for( ThreadIdType i = 0; i < this->m_NumberOfThreads; ++i )
        {
            std::vector<DerivativeType> & derivative = this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariables[ i ].st_Derivatives;
    ...
    ::ThreadedComputeDerivativeLowMemory( ThreadIdType threadId )
    {
        ...
        std::vector<DerivativeType> & derivatives = this->m_AverageConditionalMIGetValueAndDerivativePerThreadVariables[ threadId ].st_Derivatives;
...
#endif // end #ifndef _itkAverageConditionalMI_HXX__
