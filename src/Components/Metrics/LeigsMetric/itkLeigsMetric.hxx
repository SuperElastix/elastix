...
void
LeigsMetric< TFixedImage, TMovingImage >
    ::ComputeListSamplesLastDimDerivative( const ListSamplePointer & listSamplesLastDim, std::vector< FixedImagePointType > * fixedImagePointList, std::vector<vnl_vector<double> > * movingImageValueList) const
{
    const unsigned int lastDim = this->GetFixedImage()->GetImageDimension() - 1;
    const unsigned int lastDimSize = this->GetFixedImage()->GetLargestPossibleRegion().GetSize( lastDim );
    ...
    movingImageValueList->resize(this->m_NumSamplesLastDimension);

    vnl_vector<double> init(nrOfRequestedSamples,0);
    for( unsigned int s = 0; s < this->m_NumSamplesLastDimension; s++ )
    {
        (*movingImageValueList)[s] = init;
    }
    ...
    for( fiter = fbegin; fiter != fend; ++fiter )
    {
        ...
        for( unsigned int s = 0; s < this->m_NumSamplesLastDimension; s++ )
        {
            ...
            if( sampleOk )
            {
                listSamplesLastDim->SetMeasurement(  this->m_NumberOfPixelsCounted, this->m_RandomList[ s ], movingImageValueTemp );
            }
            if( sampleOk )
            {
                (*movingImageValueList)[this->m_RandomList[ s ]](this->m_NumberOfPixelsCounted) = movingImageValueTemp;
            }
...
//    for( unsigned int s = 0; s < this->m_NumSamplesLastDimension; s++ )
//    {
//        std::cout << nrOfRequestedSamples << " " << this->m_NumberOfPixelsCounted << " " << s << " " << this->m_RandomList[ s ] << " " << (*movingImageValueList)[this->m_RandomList[ s ]] <<std::endl;
//        (*movingImageValueList)[this->m_RandomList[ s ]].set_size(this->m_NumberOfPixelsCounted);
//        std::cout << nrOfRequestedSamples << " " << this->m_NumberOfPixelsCounted << " " << s << " " << this->m_RandomList[ s ] << " " << (*movingImageValueList)[this->m_RandomList[ s ]] <<std::endl;
//
//    }
}
...
//    timer.Start();
    ...
    std::vector<vnl_vector<double > > movingImageValueList;
    
    this->ComputeListSamplesLastDimDerivative(listSamplesLastDim, &(fixedImagePointList), &(movingImageValueList));
    this->m_BinaryKNNTree->SetSample( listSamplesLastDim );
    this->m_BinaryKNNTree->GenerateTree();
...
//    timer.Start();
    ...
    this->m_MovingImageValueList = &(movingImageValueList);
...
::GetValueAndDerivativeSingleThreaded( const TransformParametersType & parameters, MeasureType& value, DerivativeType& derivative ) const
{
    ...
    vnl_sparse_matrix<double> diagonal(this->m_NumberOfPixelsCounted,this->m_NumberOfPixelsCounted);
    vnl_sparse_matrix<double> laplacian(this->m_NumberOfPixelsCounted,this->m_NumberOfPixelsCounted);
    vnl_vector<double> ones(this->m_NumberOfPixelsCounted,1);
    ...
    value = eig.get_eigenvalue(1);
    vnl_vector<double> eigenvectorOne = eig.get_eigenvector(1);

    vnl_vector<double> WeightsOne;
    allWeights.mult(ones,WeightsOne);
    vnl_vector<double> vv = element_product(eigenvectorOne,eigenvectorOne);
    vnl_vector<double> vvWeights;
    allWeights.pre_mult(vv,vvWeights);
    vnl_vector<double> Weightsv;
    allWeights.mult(eigenvectorOne,Weightsv);

    for( unsigned int s = 0; s < this->m_NumSamplesLastDimension; s++ )
    {
        vnl_vector<double> vvm = element_product(vv,movingImageValueList[s]);
        vnl_vector<double> vvmWeights;
        allWeights.pre_mult(vvm,vvmWeights);
        vnl_vector<double> Weightsm;
        allWeights.mult(movingImageValueList[s],Weightsm);
        vnl_vector<double> vm = element_product(eigenvectorOne,movingImageValueList[s]);
        vnl_vector<double> Weightsvm;
        allWeights.mult(vm,Weightsvm);
        ...
        for( unsigned long j = 0; j < this->m_NumberOfPixelsCounted; j++ )
        {
            ...
            this->GetFixedImage()->TransformPhysicalPointToContinuousIndex( fixedImagePointList[j], voxelCoord );
            voxelCoord[ lastDim ] = this->m_RandomList[ s ];
            this->GetFixedImage()->TransformContinuousIndexToPhysicalPoint( voxelCoord, fixedPoint );
            ...
            float weight = -2 /this->m_Time * ((1-value)*(vvm(j)*WeightsOne(j) + vvWeights(j)*movingImageValueList[s](j)-vvmWeights(j)-vv(j)*Weightsm(j))-2*(vm(j)*Weightsv(j) - eigenvectorOne(j)*Weightsvm(j)));
...
::ThreadedComputeDerivative( ThreadIdType threadId )
{
    ...
    vnl_vector<double> ones(this->m_NumberOfPixelsCounted,1);
    vnl_vector<double> WeightsOne;
    this->m_WeightMatrix->mult(ones,WeightsOne);
    vnl_vector<double> vv = element_product(this->m_EigenVector,this->m_EigenVector);
    vnl_vector<double> vvWeights;
    this->m_WeightMatrix->pre_mult(vv,vvWeights);
    vnl_vector<double> Weightsv;
    this->m_WeightMatrix->mult(this->m_EigenVector,Weightsv);

    for( unsigned int s = 0; s < this->m_NumSamplesLastDimension; s++ )
    {
        
        vnl_vector<double> vvm = element_product(vv,(*this->m_MovingImageValueList)[s]);
        vnl_vector<double> vvmWeights;
        this->m_WeightMatrix->pre_mult(vvm,vvmWeights);
        vnl_vector<double> Weightsm;
        this->m_WeightMatrix->mult((*this->m_MovingImageValueList)[s],Weightsm);
        vnl_vector<double> vm = element_product(this->m_EigenVector,(*this->m_MovingImageValueList)[s]);
        vnl_vector<double> Weightsvm;
        this->m_WeightMatrix->mult(vm,Weightsvm);

        for( unsigned long j = pos_begin; j < pos_end; j++ )
        {
            FixedImageContinuousIndexType           voxelCoord;
            FixedImagePointType                     fixedPoint;
            MovingImagePointType                    mappedPoint;
            ...
            this->GetFixedImage()->TransformPhysicalPointToContinuousIndex( (*this->m_FixedImagePointList)[j], voxelCoord );
            voxelCoord[ lastDim ] = this->m_RandomList[ s ];
            this->GetFixedImage()->TransformContinuousIndexToPhysicalPoint( voxelCoord, fixedPoint );
            this->TransformPoint( fixedPoint, mappedPoint );
            this->EvaluateMovingImageValueAndDerivative(mappedPoint, movingImageValueTemp, &movingImageDerivativeTemp );
            ...
            float weight = -2 /this->m_Time * ((1-this->m_EigenValue)*(vvm(j)*WeightsOne(j) + vvWeights(j)*(*this->m_MovingImageValueList)[s](j)-vvmWeights(j)-vv(j)*Weightsm(j))-2*(vm(j)*Weightsv(j) - this->m_EigenVector(j)*Weightsvm(j)));
            for( unsigned int p = 0; p < nzjiTemp.size(); ++p)
            {
                derivative[ nzjiTemp[p] ] += weight*imageJacobianTemp[p];
            }
