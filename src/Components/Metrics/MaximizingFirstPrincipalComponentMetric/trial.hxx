 ...
 */
    ...
    ::GetValue( const TransformParametersType & parameters ) const
  {
    ...
    this->SetTransformParameters( parameters );
		...
		/** Compute covariance matrix K */
		MatrixType K( (AtMinusMean*AMinusMean) );
 ...
 */
	...
	MeasureType& value, DerivativeType& derivative, vnl_matrix< RealType >& imageMatrix ) const
  {
    ...
    vnl_vector< DerivativeValueType > dKiidmu( P ); //Trace of derivative of covariance matrix
		...
		DerivativeMatrixType dAdmu_v1( A.rows(), P ); //dAdmu * v1 
		DerivativeMatrixType meandAdmu_v1( A.rows(), P ); //meandAdmu * v
      ...
      {	
        ...
        dMTdmu = imageJacobian;
          ...
          meandAdmu[ d ][ nzjis[ d ][ p ] ] += dMTdmu[ p ]/A.rows();
					AtdAdmuii[ nzjis[ d ][ p ] ] += AtMinusMean[ d ][ pixelIndex ]*dMTdmu[ p ];							
				}
			} // end loop over t
		...
		for(unsigned int i = 0; i < A.rows(); i++)
		{
			for (unsigned int d = 0; d < realNumLastDimPositions; ++d )
			{
				...
				{
					meandAdmu_v1[ i ][ p ] += meandAdmu[ d ][ p ]*v1[ d ];
					meanAtdAdmuii[ p ] += AtMinusMean[ d ][ i ]*meandAdmu[ d ][ p ];
				}
			}
		}	
