...
::Initialize( void ) throw ( ExceptionObject )
{
  ...
  this->m_TransformMovingImageFilter->SetTransform( dynamic_cast<CombinationTransformType *>(
  ...
  this->m_TransformMovingImageFilter->SetDefaultPixelValue( 0.0 );
  ...
  for ( iFilter=0; iFilter<FixedImageDimension; iFilter++ )
  {
    ...
    this->m_FixedSobelFilters[iFilter]->OverrideBoundaryCondition( &m_FixedBoundCond );
    this->m_FixedSobelFilters[iFilter]->SetOperator( m_FixedSobelOperators[iFilter] );
    this->m_FixedSobelFilters[iFilter]->SetInput( m_CastFixedImageFilter->GetOutput() );
  ...
  this->m_CastMovedImageFilter->SetInput( m_TransformMovingImageFilter->GetOutput() );
  ...
  for ( iFilter=0; iFilter<MovedImageDimension; iFilter++ )
  {
    ...
    this->m_MovedSobelFilters[iFilter]->OverrideBoundaryCondition( &m_MovedBoundCond );
    this->m_MovedSobelFilters[iFilter]->SetOperator( m_MovedSobelOperators[ iFilter ] );
    this->m_MovedSobelFilters[iFilter]->SetInput( m_CastMovedImageFilter->GetOutput() );
