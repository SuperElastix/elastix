#ifndef __elxMCPANMI_HXX__
#define __elxMCPANMI_HXX__
...
namespace elastix
{
    ...
    ::BeforeRegistration( void )
    {
        ...
        std::size_t count  = this->m_Configuration
        ->CountNumberOfParameterEntries( "Channels" );

        std::vector<unsigned int> channelsVector(count,0);
        
        for (unsigned int j = 0; j < count; ++j)
        {
            unsigned int channel = j;
            this->GetConfiguration()->ReadParameter( channel, "Channels", this->GetComponentLabel(), j, 0 );
            channelsVector[j]=channel;
            
        }
        this->SetChannels( channelsVector );
...
#endif // end #ifndef _elxMCPANMI_HXX__
