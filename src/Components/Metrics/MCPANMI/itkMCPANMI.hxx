#ifndef _itkMCPANMI_HXX__
#define _itkMCPANMI_HXX__

#include "itkMCPANMI.h"
...
namespace itk
{
    ...
    MCPANMI< TFixedImage, TMovingImage >
    ::MCPANMI()
    {
        this->m_MCPANMIThreaderParameters.m_Metric = this;
        
        this->m_MCPANMIGetValueAndDerivativePerThreadVariables     = NULL;
        this->m_MCPANMIGetValueAndDerivativePerThreadVariablesSize = 0;
    ...
    ::~MCPANMI()
    {
        delete[] this->m_MCPANMIGetValueAndDerivativePerThreadVariables;
    }
    ...
    ::FillVectors(void)
    {
        ...
        for(unsigned int d = 0; d < lastDimSize; d++)
        {
            ...
            {
                if ( this->m_Channels[i] != this->m_Channels[d] )
                {
    ...
    template< class TFixedImage, class TMovingImage >
    typename MCPAverageNormalizedMutualInformationMetric< TFixedImage, TMovingImage >::MeasureType
    MCPANMI< TFixedImage, TMovingImage >
    ...
    ::GetValueAndAnalyticalDerivative( const ParametersType & parameters, MeasureType & value, DerivativeType & derivative ) const
    {
        ...
        if( this->m_SubtractMean )
        {
            ...
            else
            {
                ...
                for( unsigned int t = 0; t < this->m_NumberOfImages; ++t )
                {
                    cMean.Fill(0.0);
                    for(unsigned int c = 0; c < this->m_NumberOfChannels[t]; ++c)
                    {
                    ...
                    mean /= static_cast< double >( this->m_NumberOfChannels[t] );
                    ...
                    startd += numParametersPerLastDimension * this->m_NumberOfChannels[t];
                }
    ...
    ::ComputePDFsThreaderCallback( void * arg )
    {
        ...
        MCPANMIMultiThreaderParameterType * temp
        = static_cast< MCPANMIMultiThreaderParameterType * >( infoStruct->UserData );
    ...
    ::LaunchComputePDFsThreaderCallback( void ) const
    {
        ...
        local_threader->SetSingleMethod( this->ComputePDFsThreaderCallback, const_cast< void * >( static_cast< const void * >( &this->m_MCPANMIThreaderParameters ) ) );
    ...
    ::InitializeThreadingParameters( void ) const
    {
        ...
        if( this->m_MCPANMIGetValueAndDerivativePerThreadVariablesSize != this->m_NumberOfThreads )
        {
            delete[] this->m_MCPANMIGetValueAndDerivativePerThreadVariables;
            this->m_MCPANMIGetValueAndDerivativePerThreadVariables = new AlignedMCPANMIGetValueAndDerivativePerThreadStruct[this->m_NumberOfThreads ];
            this->m_MCPANMIGetValueAndDerivativePerThreadVariablesSize = this->m_NumberOfThreads;
        }
        
        for( ThreadIdType i = 0; i < this->m_NumberOfThreads; ++i )
        {
            this->m_MCPANMIGetValueAndDerivativePerThreadVariables[ i ].st_NumberOfPixelsCounted = NumericTraits< SizeValueType >::Zero;
            this->m_MCPANMIGetValueAndDerivativePerThreadVariables[ i ].st_NumberOfPixelsCountedVector.assign(this->m_NumSamplesLastDimension, NumericTraits< SizeValueType >::Zero);
            this->m_MCPANMIGetValueAndDerivativePerThreadVariables[ i ].st_JointPDFs.resize(lastDimSize);
            
            for(unsigned int d = 0; d < lastDimSize; d++)
            {
                JointPDFPointer & jointPDF = this->m_MCPANMIGetValueAndDerivativePerThreadVariables[ i ].st_JointPDFs[ d ];
    ...
    ::AfterThreadedComputePDFs( void ) const
    {
        ...
        for( ThreadIdType i = 0; i < this->m_NumberOfThreads; ++i )
        {
            this->m_NumberOfPixelsCounted += this->m_MCPANMIGetValueAndDerivativePerThreadVariables[ i ].st_NumberOfPixelsCounted;
            
            for(unsigned int s = 0; s < this->m_NumSamplesLastDimension; s++)
            {
                this->m_NumberOfPixelsCountedVector[this->m_RandomList[s]] += this->m_MCPANMIGetValueAndDerivativePerThreadVariables[ i ].st_NumberOfPixelsCountedVector[this->m_RandomList[s]];
            }
        ...
        for(unsigned int s = 0; s < this->m_NumSamplesLastDimension; s++)
        {
            ...
            for( ThreadIdType i = 0; i < this->m_NumberOfThreads; ++i )
            {
                itT[ i ] = JointPDFIteratorType(this-> m_MCPANMIGetValueAndDerivativePerThreadVariables[ i ].st_JointPDFs[this->m_RandomList[s]], this->m_JointPDFs[this->m_RandomList[s]]->GetBufferedRegion() );
            }
    ...
    ::ComputePDFsSingleThreaded( const ParametersType & parameters ) const
    {
        ...
        for( fiter = fbegin; fiter != fend; ++fiter )
        {
            ...
            if ( numSamplesOk > 1)
            {
                ...
                bool MC = false;
                unsigned int ref = this->m_Channels[positions[0]];
                for( unsigned int o = 0; o < numSamplesOk; o++ )
                {
                    MC |= (this->m_Channels[positions[o]] != ref);
    ...
    ::ThreadedComputePDFs( ThreadIdType threadId )
    {
        ...
        std::vector<JointPDFPointer> & jointPDFs = this->m_MCPANMIGetValueAndDerivativePerThreadVariables[ threadId ].st_JointPDFs;
        std::vector<SizeValueType> & NumberOfPixelsCountedVector = this->m_MCPANMIGetValueAndDerivativePerThreadVariables[ threadId ].st_NumberOfPixelsCountedVector;
        SizeValueType & NumberOfPixelsCounted = this->m_MCPANMIGetValueAndDerivativePerThreadVariables[ threadId ].st_NumberOfPixelsCounted;
    ...
    ::LaunchComputeDerivativeLowMemoryThreaderCallback( void ) const
    {
        typename ThreaderType::Pointer local_threader = ThreaderType::New();
        local_threader->SetNumberOfThreads( this->m_NumberOfThreads );
        local_threader->SetSingleMethod( this->ComputeDerivativeLowMemoryThreaderCallback, const_cast< void * >( static_cast< const void * >( &this->m_MCPANMIThreaderParameters ) ) );
    ...
    ::UpdateJointPDFAndDerivatives(const std::vector<double> & fixedImageValue, const std::vector<JointPDFPointer> * jointPDFs, const std::vector<DerivativeType> * imageJacobian, const std::vector<NonZeroJacobianIndicesType> * nzji, const unsigned int n, const std::vector<unsigned int> & positions  ) const
    {
        ...
        for (unsigned int o = 0; o < n; o++)
        {
            ...
            for (unsigned int k = 0; k < n; k++)
            {
                ...
                if( this->m_Channels[positions[o]] != this->m_Channels[positions[k]] )
                {
                    ...
                    if( !imageJacobian )
                    {
                        for( unsigned int f = 0; f < fixedParzenValues[o].GetSize(); f++ )
                        {
                            ...
                            for( unsigned int m = 0; m < movingParzenValues.GetSize(); m++ )
                            {
                                ...
                                if(n!=this->m_NumberOfChannels[this->m_Channels[positions[o]]])
                                {
                                    it.Value() += static_cast< PDFValueType >( fv * mv / static_cast<double>(n-this->m_NumberOfChannels[this->m_Channels[positions[o]]]));
                                }
                                ++it;
                            }
                            it.NextLine();
    ...
    ::UpdateJointPDFDerivatives( const JointPDFIndexType & pdfIndex, double factor, double factorr, const DerivativeType & imageJacobianFix, const NonZeroJacobianIndicesType & nzjiFix, const DerivativeType & imageJacobianMov, const NonZeroJacobianIndicesType & nzjiMov, const unsigned int o, const std::vector<unsigned int> & positions, const unsigned int n) const
    {
        ...
        for ( unsigned int i = 0; i < nzjiFix.size(); i++)
        {
            ...
            *( ptr ) -= static_cast< PDFDerivativeValueType >( (*imjac) * (factorr / static_cast<double>(n - this->m_NumberOfChannels[this->m_Channels[positions[o]]]) ) );
            imjac++;
        ...
        for ( unsigned int i = 0; i < nzjiMov.size(); i++)
        {
            ...
            *( ptr ) -= static_cast< PDFDerivativeValueType >( (*imjac) * (factor / static_cast<double>(n - this->m_NumberOfChannels[this->m_Channels[positions[o]]]) ) );
            imjac++;
            ...
            for (unsigned int k = 0; k < n; k++)
            {
                if( this->m_Channels[positions[o]] != this->m_Channels[positions[k]] )
                {
                    ...
                    for ( unsigned int i = 0; i < (*nzji)[o].size(); i++)
                    {
                        ...
                        if(n!=this->m_NumberOfChannels[this->m_Channels[positions[o]]])
                        {
                            derivative[ mu ] += static_cast< PDFDerivativeValueType >( (*imjac) * (extraSum / static_cast<double>(n - this->m_NumberOfChannels[this->m_Channels[positions[o]]]) ) );
                        }
                        imjac++;
                    ...
                    for ( unsigned int i = 0; i < (*nzji)[k].size(); i++)
                    {
                        ...
                        if(n!=this->m_NumberOfChannels[this->m_Channels[positions[o]]])
                        {
                            derivative[ mu ] += static_cast< PDFDerivativeValueType >( (*imjac) * (regSum / static_cast<double>(n - this->m_NumberOfChannels[this->m_Channels[positions[o]]]) ) );
                        }
                        imjac++;
...
#endif // end #ifndef _itkMCPANMI_HXX__
