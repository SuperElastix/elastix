...
#ifndef PCAMetric3_HXX
#define PCAMetric3_HXX

#include <numeric>
#include <fstream>
#include <math.h>
#include "itkPCAMetric3.h"
...
PCAMetric3<TFixedImage,TMovingImage>
::PCAMetric3():
...
template <class TFixedImage, class TMovingImage>
typename PCAMetric3<TFixedImage,TMovingImage>::MeasureType
PCAMetric3<TFixedImage,TMovingImage>
                         ...
                         MeasureType& value, DerivativeType& derivative ) const
{
    ...
    //DerivativeMatrixType invK = vnl_cholesky(K).inverse();
    //DerivativeMatrixType SAtmm(S*Atmm);
...
//                {
//                    derivative[ nzjis[ d ][ p ] ] += invK[ z ][ d ] * ( SAtmm[ z ][ pixelIndex ] * dMTdmu[ p ] * S( d, z ) +
//                                                          dSdmu_part1( z, d ) * Atmm[ d ][ pixelIndex ] * dMTdmu[ p ] * CS[ d ][ z ] );
//                }
    ...
    for ( pixelIndex = 0; pixelIndex < SamplesOK.size(); ++pixelIndex )
    {
        ...
        for ( unsigned int d = 0; d < G; ++d )
        {
            ...
            for( unsigned int p = 0; p < nzjis[ d ].size(); ++p)
            {
                for(unsigned int z = 0; z < G; z++)
                {
                    derivative[ nzjis[ d ][ p ] ] += (1.0/eig.get_eigenvalue(G-z-1)) * (vSAtmm[ z ][ pixelIndex ] * dMTdmu[ p ] * Sv[ d ][ z ] +
