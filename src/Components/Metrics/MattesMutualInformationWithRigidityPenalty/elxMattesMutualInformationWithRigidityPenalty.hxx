...
::MattesMutualInformationWithRigidityPenalty()
{
  ...
  this->m_MattesMutualInformationMetric->SetUseDerivative( true );
...
::BeforeRegistration( void )
{
  ...
  if ( fixedRigidityImageName != "" )
  {
    ...
    catch( ExceptionObject & excp )
    {
      ...
      err_str += "\nError occurred while reading the FixedRigidityImage.\n";
  ...
  if ( movingRigidityImageName != "" )
  {
    ...
    catch( ExceptionObject & excp )
    {
      ...
      excp.SetLocation( "MattesMutualInformationWithRigidityPenalty - BeforeRegistration()" );
      std::string err_str = excp.GetDescription();
      err_str += "\nError occurred while reading the MovingRigidityImage.\n";
      excp.SetDescription( err_str );
      /** Pass the exception to an higher level. */
      throw excp;
    }
