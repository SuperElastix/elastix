#ifndef _itkPJEMetric_HXX__
#define _itkPJEMetric_HXX__
...
namespace itk
{
    ...
    PJEMetric< TFixedImage, TMovingImage >
    ::PJEMetric()
    {
        ...
        while( fixedPDFit != fixedPDFend )
        {
            ...
            while( movingPDFit != movingPDFend )
            {
                ...
                if( jointPDFValue > 1e-16 && fixedPDFValue > 1e-16 )
                {
                    ...
                    (*(this->m_PRatioArray[this->m_RandomList[n]]))[ fixedIndex ][ movingIndex ] = static_cast< PRatioType >(pRatio/static_cast<double>(this->m_NumberOfPixelsCountedVector[this->m_RandomList[n]]) / static_cast<float>(this->m_NumSamplesLastDimension));
...
#endif // end #ifndef _itkPJEMetric_HXX__
