...
::AdvancedKappaStatisticImageToImageMetric()
{
  ...
  this->m_ThreaderAreaSum.resize( 0 );
  this->m_ThreaderAreaIntersection.resize( 0 );
...
::InitializeThreadingParameters( void ) const
{
  ...
  this->m_ThreaderAreaSum.resize( this->m_NumberOfThreads, 0 );
  this->m_ThreaderAreaIntersection.resize( this->m_NumberOfThreads, 0 );

  /** Initialize the derivatives. */
  this->m_ThreaderDerivativeSum1.resize( this->m_NumberOfThreads );
  this->m_ThreaderDerivativeSum2.resize( this->m_NumberOfThreads );
  for( ThreadIdType i = 0; i < this->m_NumberOfThreads; ++i )
  {
    this->m_ThreaderDerivativeSum1[ i ].SetSize( this->GetNumberOfParameters() );
    this->m_ThreaderDerivativeSum1[ i ].Fill( NumericTraits<DerivativeValueType>::Zero );
...
::ThreadedGetValueAndDerivative( ThreadIdType threadId )
{
  ...
  DerivativeType & vecSum1 = this->m_ThreaderDerivativeSum1[ threadId ];
  DerivativeType & vecSum2 = this->m_ThreaderDerivativeSum2[ threadId ];
  ...
  this->m_ThreaderAreaSum[ threadId ] = fixedForegroundArea + movingForegroundArea;
  this->m_ThreaderAreaIntersection[ threadId ] = intersection;
  ...
  MeasureType & value, DerivativeType & derivative ) const
{
  ...
  for( ThreadIdType i = 0; i < this->m_NumberOfThreads; ++i )
  {
    areaSum      += this->m_ThreaderAreaSum[ i ];
    intersection += this->m_ThreaderAreaIntersection[ i ];
  }
  ...
  {
    DerivativeType vecSum1 = this->m_ThreaderDerivativeSum1[ 0 ];
    DerivativeType vecSum2 = this->m_ThreaderDerivativeSum2[ 0 ];
    for( ThreadIdType i = 1; i < this->m_NumberOfThreads; ++i )
    {
      vecSum1 += this->m_ThreaderDerivativeSum1[ i ];
      vecSum2 += this->m_ThreaderDerivativeSum2[ i ];
    }
    derivative = tmp1 * vecSum1 - tmp2 * vecSum2;
  }
  else // multi-threaded
...
::AccumulateDerivativesThreaderCallback( void * arg )
{
  ...
  for( unsigned int j = jmin; j < jmax; j++ )
  {
    ...
    {
      sum1 += temp->st_Metric->m_ThreaderDerivativeSum1[ i ][ j ];
      sum2 += temp->st_Metric->m_ThreaderDerivativeSum2[ i ][ j ];
    }
