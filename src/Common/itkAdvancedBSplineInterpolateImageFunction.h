 ...
 *
 *=========================================================================*/
#ifndef __itkAdvancedBSplineInterpolateImageFunction_h
#define __itkAdvancedBSplineInterpolateImageFunction_h
...
class AdvancedBSplineInterpolateImageFunction:
  public InterpolateImageFunction< TImageType, TCoordRep >
{
public:
  /** Standard class typedefs. */
  typedef AdvancedBSplineInterpolateImageFunction                   Self;
  typedef InterpolateImageFunction< TImageType, TCoordRep > Superclass;
  typedef SmartPointer< Self >                              Pointer;
  typedef SmartPointer< const Self >                        ConstPointer;

  /** Run-time type information (and related methods). */
  itkTypeMacro(AdvancedBSplineInterpolateImageFunction, InterpolateImageFunction);
  ...
  MatrixType EvaluateHessian(const PointType & point) const
  {
    ...
    this->GetInputImage()->TransformPhysicalPointToContinuousIndex(point,
                                                                   index);
    // No thread info passed in, so call method that doesn't need thread ID.
    return ( this->EvaluateHessianAtContinuousIndex(index) );
  }

  MatrixType EvaluateHessian(const PointType & point,
                                         ThreadIdType threadId) const
  {
    ContinuousIndexType index;
    ...
    this->GetInputImage()->TransformPhysicalPointToContinuousIndex(point,
                                                                   index);
    return ( this->EvaluateHessianAtContinuousIndex(index, threadId) );
  }

  MatrixType EvaluateHessianAtContinuousIndex(
    const ContinuousIndexType & x) const
  {
    // Don't know thread information, make evaluateIndex, weights,
    // weightsDerivative
    ...
    vnl_matrix< double > weights( ImageDimension, ( m_SplineOrder + 1 ) );
    vnl_matrix< double > weightsDerivative( ImageDimension, ( m_SplineOrder + 1 ) );
    vnl_matrix< double > weightsHessian( ImageDimension, (m_SplineOrder + 1));
  ...
  void EvaluateValueAndDerivativeAndHessian(const PointType & point,
                                  OutputType & value,
                                  CovariantVectorType & deriv,
                                  MatrixType & hessian) const
  {
    ...
    // No thread info passed in, so call method that doesn't need thread ID.
    this->EvaluateValueAndDerivativeAndHessianAtContinuousIndex(index,
                                                      value,
                                                      deriv,
                                                      hessian);
  }
    ...
    ) const
  {
    ...
    // good
    // as long as this method is in scope.
    this->EvaluateValueAndDerivativeAndHessianAtContinuousIndexInternal(x,
                                                              value,
                                                              deriv,
                                                              hessian,
                                                              evaluateIndex,
...
protected:
  ...
  virtual void EvaluateValueAndDerivativeAndHessianAtContinuousIndexInternal(const ContinuousIndexType & x,
                                                                   OutputType & value,
                                                                   CovariantVectorType & derivativeValue,
                                                                   MatrixType & hessian,
                                                                   vnl_matrix< long > & evaluateIndex,
                                                                   vnl_matrix< double > & weights,
                                                                   vnl_matrix< double > & weightsDerivative,
                                                                   vnl_matrix< double > & weightsHessian
                                                                   ) const;
  ...
  virtual MatrixType EvaluateHessianAtContinuousIndexInternal(const ContinuousIndexType & x,
                                                                          vnl_matrix< long > & evaluateIndex,
                                                                          vnl_matrix< double > & weights,
                                                                          vnl_matrix< double > & weightsDerivative,
                                                                          vnl_matrix< double > & weightsHessian
  ...
  AdvancedBSplineInterpolateImageFunction();
  ~AdvancedBSplineInterpolateImageFunction();
...
private:
  AdvancedBSplineInterpolateImageFunction(const Self &); //purposely not implemented
  ...
  void SetHessianWeights(const ContinuousIndexType & x,
                            const vnl_matrix< long > & EvaluateIndex,
                            vnl_matrix< double > & weights,
                            unsigned int splineOrder) const;
...
#include "itkAdvancedBSplineInterpolateImageFunction.hxx"
#endif
