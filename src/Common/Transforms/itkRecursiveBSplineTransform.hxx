        ...
        bool & inside ) const
{
    ...
    for (unsigned int j = 0; j < SpaceDimension; ++j)
    {
        ...
        unsigned int c = 0;
        outputPoint[j] = static_cast<TScalar>(interpolateFunction< SpaceDimension, SplineOrder, TScalar>
                                                 ::interpolate(basePointer,
                                                               steps,
                                                               weightsPointer,
                                                               basePointer,
                                                               indices,
...
template< class TScalar, unsigned int NDimensions, unsigned int VSplineOrder >
typename RecursiveBSplineTransform< TScalar, NDimensions, VSplineOrder >::NumberOfParametersType
RecursiveBSplineTransform< TScalar, NDimensions, VSplineOrder >
...
{
    std::cout << "Calling GetSpatialJacobian..." << std::endl;
    /** Convert the physical point to a continuous index, which
   * is needed for the 'Evaluate()' functions below.
   */
    ContinuousIndexType cindex;
...
//    }   // end for i
    ...
    for (unsigned int j = 0; j < SpaceDimension; ++j)
    {
        ...
        const TScalar *basePointer = this->m_CoefficientImages[j]->GetBufferPointer();
        interpolateFunction< SpaceDimension, SplineOrder, TScalar>
                                                 ::interpolateValueAndDerivative(derivativeValue,
                                                                                 basePointer,
                                                                                 steps,
                                                                                 weightsPointer,
...
void
RecursiveBSplineTransform< TScalarType, NDimensions, VSplineOrder >
::GetSpatialHessian(
        const InputPointType & ipp,
