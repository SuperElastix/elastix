...
::AdvancedBSplineDeformableTransform():Superclass(SpaceDimension,0)
{
  ...
  this->m_PointToIndex = this->m_IndexToPoint.GetInverse();
...
::PrintSelf(std::ostream &os, Indent indent) const
{
  ...
  os << indent << "PointToIndex: " << this->m_PointToIndex << std::endl;
    ...
    while ( ! iterator[0].IsAtEnd() )
      ...
      for ( j = 0; j < SpaceDimension; j++ )
        ...
        outputPoint[j] += static_cast<ScalarType>( 
          weights[counter] * iterator[j].Get());
        }
...
::TransformPointToContinuousGridIndex( const InputPointType & point, ContinuousIndexType & index ) const
{
  ...
  cvector = this->m_PointToIndex * tvector;
  ...
  SpatialHessianType & sh ) const
{
  ...
  // NOTE: if the support region does not lie totally within the grid
  // we assume zero displacement and return
  if ( !this->InsideValidRegion( cindex ) )
  {
    return;
  }

  /** Resize the vector of matrices. */
  ...
  NonZeroJacobianIndicesType & nonZeroJacobianIndices ) const
{
  ...
  for ( unsigned int mu = 0; mu < numberOfNonZeroJacobianIndices; ++mu )
  {
    jsh[ mu ].resize( SpaceDimension );
    // set all matrices to 0
    /*for ( unsigned int dim = 0; dim < SpaceDimension; ++dim )
    {
      jsh[ mu ][ dim ].Fill( 0 );
    } // Not required, since the default constructor sets everything to zero. */
  }
  ...
  /** Get the support region. */
  RegionType supportRegion;
  //supportRegion.SetSize( this->m_WeightsFunction->GetSupportSize() );
  supportRegion.SetSize( this->m_SupportSize );
  supportRegion.SetIndex( supportIndex );
