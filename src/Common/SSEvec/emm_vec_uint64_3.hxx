...
template<> vec<uint64_t, 3>  vec<uint64_t, 3>::loada( const uint64_t * v ) {
  //return vec( v ); // can't do aligned more efficient. 
  return vec<uint64_t,3>( _mm_load_si128( reinterpret_cast<const __m128i *>( v ) ) , _mm_load_si128( reinterpret_cast<const __m128i *>( v +2 ) )  );
}
