...
#define cudaCheckMsg(msg) __cudaCheckMsg(msg, __FILE__, __LINE__)
	...
	inline cudaError_t cudaMemcpy(void* dst, const void* src, size_t nof_elems, size_t sizeof_elem, cudaMemcpyKind direction) {
	...
	template <class T>
	inline void cudaMemcpy(T* dst, const T* src, size_t nof_elems, cudaMemcpyKind direction) {
		size_t size = nof_elems * sizeof(T);
	...
	inline cudaError_t cudaMemcpyToSymbol(T& dst, const Q& src, cudaMemcpyKind direction) {
	...
	template <typename T>
	inline cudaError_t cudaBindTextureToArray(T& tex, cudaArray* array, const cudaChannelFormatDesc desc) {
	...
	/* in the future we might wrap the Driver-API around this so we can keep using the high-level API */
