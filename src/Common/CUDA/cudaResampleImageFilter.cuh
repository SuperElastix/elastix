...
namespace cuda
{
	...
	TOutputImageType* cudaCastToType(cudaExtent& volumeExtent, const TInputImageType* src, TOutputImageType* dst, cudaMemcpyKind direction, int device);
	...
	public:
		...
		void cudaInit();
		void cudaUnInit();
		void cudaCopyImageSymbols(float3& InputImageSpacing, float3& InputImageOrigin, float3& OutputImageSpacing, float3& OutputImageOrigin, float DefaultPixelValue);
...
#if defined(__CUDACC__)
		template <typename tex_t> cudaError_t cudaBindTextureToArray(cudaArray* dst, const TInternalImageType* src, cudaExtent& extent, tex_t& tex, cudaChannelFormatDesc& desc, bool normalized = false, bool onDevice = false);
#endif /* __CUDACC__ */
	};
