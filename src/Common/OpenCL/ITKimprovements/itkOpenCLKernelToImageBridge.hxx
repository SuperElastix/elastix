...
} // end of ITKOpenCLKernelToImageBridge namespace
  ...
  const bool updateCPU )
{
  ...
  {
    error = kernel.SetArg( argumentIndex,
      imageDataManager->GetGPUBufferPointer(), sizeof( cl_mem ) );
  }
  else
  {
  ...
  if( error != CL_SUCCESS )
  {
    itkOpenCLWarningMacroGeneric( << "Setting kernel argument failed with "
                                  << "OpenCLKernelToImageBridge::SetImage(kernel name: '"
                                  << kernel.GetName() << "', " << argumentIndex << ")" );
  }
  ...
  const bool updateCPU )
{
  ...
  OpenCLKernelToImageBridge< TImage >::SetImageDataManager(
    kernel, argumentIndex, image->GetGPUDataManager(), updateCPU );
}
  ...
  typename GPUDataManager::Pointer & imageMetaDataManager )
{
  ...
  OpenCLKernelToImageBridge< TImage >::SetImageDataManager( kernel,
    argumentIndex, imageMetaDataManager, false );
}
