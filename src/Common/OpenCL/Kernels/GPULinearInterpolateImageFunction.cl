                                      ...
                                      __constant GPUImageFunction1D *image_function)
{
  ...
  if(basei < image_function->start_index)
  {
    basei = image_function->start_index;
  }
  ...
  basei = basei + 1;
  if(basei > image_function->end_index)
  {
    return val0;
  }
                                      ...
                                      __constant GPUImageFunction2D *image_function)
{
  ...
  if(basei.x < image_function->start_index.x)
  {
    basei.x = image_function->start_index.x;
  }
  const float distance0 = index.x - (float)(basei.x);
  if(basei.y < image_function->start_index.y)
  {
    basei.y = image_function->start_index.y;
  }
  ...
  else if(distance1 <= 0.) // if they have the same "y"
  {
    ...
    if(basei.x > image_function->end_index.x)
    {
      return val00;
    }
  ...
  else if(distance0 <= 0.) // if they have the same "x"
  {
    ...
    if(basei.y > image_function->end_index.y)
    {
      return val00;
    }
  ...
  basei.x = basei.x + 1;
  if(basei.x > image_function->end_index.x) // interpolate across "y"
  {
    basei.x = basei.x - 1;
    basei.y = basei.y + 1;
    if(basei.y > image_function->end_index.y)
  ...
  basei.y = basei.y + 1;
  if(basei.y > image_function->end_index.y) // interpolate across "x"
  {
    return valx0;
  }
                                      ...
                                      __constant GPUImageFunction3D *image_function)
{
  ...
  if(basei.z < image_function->start_index.z)
  {
    basei.z = image_function->start_index.z;
  }
  ...
  else
  {
    if(distance1 <= 0.)
    {
      ...
      {
        basei.z = basei.z + 1;
        if(basei.z > image_function->end_index.z)
        {
          return val000;
        }
      ...
      {
        basei.x = basei.x + 1;
        if(basei.x > image_function->end_index.x) // interpolate across "z"
        {
          basei.x = basei.x - 1;
          basei.z = basei.z + 1;
          if(basei.z > image_function->end_index.z)
        ...
        const float valx00 = mad(distance0, (val100 - val000), val000);
        basei.z = basei.z + 1;
        if(basei.z > image_function->end_index.z) // interpolate across "x"
        {
          return valx00;
        }
    ...
    {
      basei.y = basei.y + 1;
      if(basei.y > image_function->end_index.y) // interpolate across "z"
      {
        basei.y = basei.y - 1;
        basei.z = basei.z + 1;
        if(basei.z > image_function->end_index.z)
      ...
      basei.z = basei.z + 1;
      if(basei.z > image_function->end_index.z) // interpolate across "y"
      {
        return val0x0;
      }
    ...
    {
      basei.x = basei.x + 1;
      if(basei.x > image_function->end_index.x) // interpolate across "yz"
      {
        basei.x = basei.x - 1;
        basei.y = basei.y + 1;
        if(basei.y > image_function->end_index.y)  // interpolate across "z"
      ...
      const float valx00 = mad(distance0, (val100 - val000), val000);
      basei.y = basei.y + 1;
      if(basei.y > image_function->end_index.y) // interpolate across "xz"
      {
        basei.y = basei.y - 1;
        basei.z = basei.z + 1;
        if(basei.z > image_function->end_index.z) // interpolate across "x"
      ...
      basei.z = basei.z + 1;
      if(basei.z > image_function->end_index.z) // interpolate across "xy"
      {
        return valxx0;
      }
      const float val011 = get_pixel_3d(basei, in, image);
