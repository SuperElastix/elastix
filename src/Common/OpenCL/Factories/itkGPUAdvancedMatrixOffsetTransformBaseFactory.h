...
class GPUAdvancedMatrixOffsetTransformBaseFactory2 : public GPUObjectFactoryBase< NDimentions >
{
public:
  ...
  void operator()( void )
  {
    this->RegisterOverride(
      typeid( AdvancedMatrixOffsetTransformBase< TType, VImageDimension > ).name(),
      typeid( GPUAdvancedMatrixOffsetTransformBase< TType, VImageDimension > ).name(),
      "GPU AdvancedMatrixOffsetTransformBase override", true,
...
private:
  ...
  GPUAdvancedMatrixOffsetTransformBaseFactory2( const Self & ); // purposely not implemented
