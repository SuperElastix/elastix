...
class GPUIdentityTransformFactory2 : public GPUObjectFactoryBase< NDimentions >
{
public:
  ...
  void operator()( void )
  {
    this->RegisterOverride(
      typeid( IdentityTransform< TType, VImageDimension > ).name(),
      typeid( GPUIdentityTransform< TType, VImageDimension > ).name(),
      "GPU IdentityTransform override", true,
...
private:
  ...
  GPUIdentityTransformFactory2( const Self & ); // purposely not implemented
