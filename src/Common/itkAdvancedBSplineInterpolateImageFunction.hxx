 ...
 *
 *=========================================================================*/
#ifndef __itkAdvancedBSplineInterpolateImageFunction_hxx
#define __itkAdvancedBSplineInterpolateImageFunction_hxx

#include "itkAdvancedBSplineInterpolateImageFunction.h"
...
::SetHessianWeights(const ContinuousIndexType & x,
                       ...
                       unsigned int splineOrder) const
{
  int hessianSplineOrder = (int)splineOrder - 2;
  switch ( hessianSplineOrder )
  {
    case 0:
    {
      for( unsigned int n = 0; n < ImageDimension; n++)
    ...
    case 1:
    {
      for ( unsigned int n = 0; n < ImageDimension; n++ )
      {
          double u = x[ n ] + 1.0 - (double)EvaluateIndex[n][1];;
          weights[n][ 0 ] = -u + 2.0;
          weights[n][ 1 ] = 3.0 * u - 5.0;
          weights[n][ 2 ] = -3.0 * u + 4.0;
          weights[n][ 3 ] = u - 1.0;
      }
    }
  }
                                                      ...
                                                      ) const
{
  ...
  SetDerivativeWeights(x,
                       ...
                       ( weightsDerivative ),
                       m_SplineOrder);
  SetHessianWeights(x, (evaluateIndex), (weightsHessian), m_SplineOrder);
  ...
  {
    IndexToPointMatrix[ i ][ i ] *= this->GetInputImage()->GetSpacing()[ i ];
  }

  MatrixType PointToIndexMatrix = static_cast< MatrixType >(IndexToPointMatrix.GetInverse());
  ...
  for ( n = 1; n < ImageDimension; n++ )
  {
    ...
    for(unsigned int i = n; i < ImageDimension; ++i)
    {
      ...
      {
        w1 = 1.0;
        w2 = 1.0; //w2 covers the diagonal elements of the hessian
        w3 = 1.0; //w3 covers the off-diagonal elements of the hessian
        for ( n1 = 0; n1 < ImageDimension; n1++ )
        {
          indx = m_PointsToIndex[p][n1];
          coefficientIndex[n1] = ( evaluateIndex )[n1][indx];
          ...
          if(n1 != (n-1) && n1 != i && i != (n-1))
          {
            w3 *= ( weights )[n1][indx];
          }
          else
          {
            w3 *= ( weightsDerivative )[n1][indx];
          }
        }

        if( i != (n-1) )
        {
          hessian(n-1,i) += m_Coefficients->GetPixel(coefficientIndex)
                        * w3;
  ...
  if ( this->m_UseImageDirection )
  {
    ...
    //hessian = static_cast< MatrixType >( PointToIndexMatrix.GetTranspose() )* ( hessian * PointToIndexMatrix );
  }
...
::MatrixType
AdvancedBSplineInterpolateImageFunction< TImageType, TCoordRep, TCoefficientType >
::EvaluateHessianAtContinuousIndexInternal(const ContinuousIndexType & x,
                                                      vnl_matrix< long > & evaluateIndex,
                                                      vnl_matrix< double > & weights,
                                                      vnl_matrix< double > & weightsDerivative,
                                                      vnl_matrix< double > & weightsHessian
