  ...
  -> NumberOfParametersType
{
  ...
  // of pixels in the grid region.
  return (static_cast<NumberOfParametersType>(m_GridRegion.GetNumberOfPixels()));
}
...
AdvancedBSplineDeformableTransformBase<TScalarType, NDimensions>::UpdatePointIndexConversions()
{
  ...
  m_PointToIndexMatrix = m_IndexToPoint.GetInverse();
  m_PointToIndexMatrixTransposed = m_PointToIndexMatrix.GetTranspose();
  m_PointToIndexMatrixIsDiagonal = true;
  for (unsigned int i = 0; i < SpaceDimension; ++i)
  {
    for (unsigned int j = 0; j < SpaceDimension; ++j)
    {
      m_PointToIndexMatrix2[i][j] = static_cast<ScalarType>(m_PointToIndexMatrix[i][j]);
      m_PointToIndexMatrixTransposed2[i][j] = static_cast<ScalarType>(m_PointToIndexMatrixTransposed[i][j]);
      if (i != j && m_PointToIndexMatrix[i][j] != 0.0)
      {
        m_PointToIndexMatrixIsDiagonal = false;
      }
    }
  }

  m_PointToIndexMatrixDiagonal = m_PointToIndexMatrixTransposed2.GetVnlMatrix().get_diagonal().data_block();
  for (unsigned int i = 0; i < SpaceDimension; ++i)
  {
    for (unsigned int j = 0; j < SpaceDimension; ++j)
    {
      m_PointToIndexMatrixDiagonalProducts[i + SpaceDimension * j] =
        m_PointToIndexMatrixDiagonal[i] * m_PointToIndexMatrixDiagonal[j];
    }
  }
...
{
  SizeType gridSize = m_GridRegion.GetSize();
  m_GridOffsetTable.Fill(1);
  for (unsigned int j = 1; j < SpaceDimension; ++j)
  {
    m_GridOffsetTable[j] = m_GridOffsetTable[j - 1] * gridSize[j - 1];
  }
...
AdvancedBSplineDeformableTransformBase<TScalarType, NDimensions>::WrapAsImages()
{
   ...
   */
  auto *       dataPointer = const_cast<PixelType *>((m_InputParametersPointer->data_block()));
...
AdvancedBSplineDeformableTransformBase<TScalarType, NDimensions>::SetCoefficientImages(ImagePointer images[])
{
  if (images[0])
  {
    ...
    for (unsigned int j = 0; j < SpaceDimension; ++j)
    {
      m_CoefficientImages[j] = images[j];
    }

    // Clean up buffered parameters
    m_InternalParametersBuffer = ParametersType(0);
    m_InputParametersPointer = nullptr;
  }
}
...
AdvancedBSplineDeformableTransformBase<TScalarType, NDimensions>::PrintSelf(std::ostream & os, Indent indent) const
{
  ...
  os << indent << "GridDirection:\n" << m_GridDirection << std::endl;
  os << indent << "GridOffsetTable: " << m_GridOffsetTable << std::endl;
  os << indent << "IndexToPoint:\n" << m_IndexToPoint << std::endl;
  os << indent << "PointToIndex:\n" << m_PointToIndexMatrix << std::endl;
  os << indent << "PointToIndex2:\n" << m_PointToIndexMatrix2 << std::endl;
  os << indent << "PointToIndexTransposed:\n" << m_PointToIndexMatrixTransposed << std::endl;
  os << indent << "PointToIndexTransposed2:\n" << m_PointToIndexMatrixTransposed2 << std::endl;

  os << indent << "CoefficientImage: [ " << m_CoefficientImages[0].GetPointer();
  for (unsigned int j = 1; j < SpaceDimension; ++j)
  {
    os << ", " << m_CoefficientImages[j].GetPointer();
  }
  os << " ]" << std::endl;

  os << indent << "WrappedImage: [ " << m_WrappedImage[0].GetPointer();
  for (unsigned int j = 1; j < SpaceDimension; ++j)
  {
  ...
  const ContinuousIndexType & index) const
{
  ...
  for (unsigned int j = 0; j < SpaceDimension; ++j)
  {
    if (index[j] < m_ValidRegionBegin[j] || index[j] >= m_ValidRegionEnd[j])
    {
      inside = false;
      break;
    }
  ...
  const InputPointType & point) const -> ContinuousIndexType
{
  ...
  Vector<double, SpaceDimension> cvector = m_PointToIndexMatrix * tvector;
