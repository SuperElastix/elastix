                                                ...
                                                const std::string & extraBuildOptions)
{
  ...
  OpenCLProgram                 program = this->m_Context->BuildProgramFromSourceCode(
    devices, sourceCode, prefixSourceCode, postfixSourceCode, extraBuildOptions);
...
{
  const std::list<OpenCLDevice> devices = this->m_Context->GetDevices();
  OpenCLProgram                 program = this->m_Context->BuildProgramFromSourceFile(
    devices, fileName, prefixSourceCode, postfixSourceCode, extraBuildOptions);
...
//------------------------------------------------------------------------------
bool
OpenCLKernelManager::SetKernelArgWithImage(const std::size_t             kernelId,
                                           const cl_uint                 argId,
                                           const GPUDataManager::Pointer manager)
{
  ...
  {
    error =
      clSetKernelArg(this->GetKernel(kernelId).GetKernelId(), argId, sizeof(cl_mem), manager->GetGPUBufferPointer());
  }
  else
  {
