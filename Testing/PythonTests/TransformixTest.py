...
class TransformixTestCase(unittest.TestCase):
    ...
    def assert_equal_image_info(self, actual: sitk.Image, expected: sitk.Image) -> None:
        ...
        self.assertEqual(actual.GetPixelIDTypeAsString(), expected.GetPixelIDTypeAsString())


    def test_without_arguments(self) -> None:
        """Tests executing transformix without arguments"""
    
        completed = subprocess.run([str(self.transformix_exe_file_path)], capture_output=True)
        self.assertEqual(completed.returncode, 0)
        self.assertEqual(completed.stderr, b'')
        self.assertEqual(completed.stdout.decode().strip(), 'Use "transformix --help" for information about transformix-usage.')


    def test_help(self) -> None:
        """Tests --help"""
    
        completed = subprocess.run([str(self.transformix_exe_file_path), "--help"], capture_output=True)
        self.assertEqual(completed.returncode, 0)
        self.assertEqual(completed.stderr, b'')
        self.assertTrue('transformix applies a transform on an input image and/or' in completed.stdout.decode())
    ...
    def test_translation(self) -> None:
        ...
        completed = subprocess.run([
          str(self.transformix_exe_file_path), "-in",
          str(data_directory_path / "2D_2x2_square_object_at_(2,1).mhd"),
          "-tp", str(parameter_directory_path / "Translation(1,-2).txt"),
          "-out", str(output_directory_path) ], capture_output=True)
        self.assertEqual(completed.returncode, 0)

        expected_image = sitk.ReadImage(str(data_directory_path/"2D_2x2_square_object_at_(1,3).mhd"))
    ...
    def test_translation_of_points(self) -> None:
        ...
        source_directory_path = pathlib.Path(__file__).resolve().parent
        test_function_output_directory_path = self.create_test_function_output_directory()
        ...
        completed = subprocess.run([
          str(self.transformix_exe_file_path), "-def",
          str(data_directory_path / "2D_unit_square_corner_points.txt"),
          "-tp", str(parameter_directory_path / "Translation(1,-2).txt"),
          "-out", str(out_directory_path) ], capture_output=True)
        self.assertEqual(completed.returncode, 0)

        outputpoints_filename = "outputpoints.txt"
        self.assertTrue(filecmp.cmp(out_directory_path  / outputpoints_filename, source_directory_path / "ExpectedOutput" / outputpoints_filename, shallow=False))
