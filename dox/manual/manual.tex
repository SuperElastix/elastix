...
\hat \vT_{\vmu} &= \arg \min_{\vT_{\vmu}}
 \CC(\vT_{\vmu} ; I_F,I_M), \label{eq:towardsparametric}
\end{align}
...
components can be recognised from
Equations~(\ref{eq:registration1})-(\ref{eq:parametric}); some
will be introduced later. First of all, we have the images. The
concept of an image needs to be defined. This is done in
...
Another thing, not immediately clear from Equation
...
\begin{split}
  p(f,m;\vmu)
  &=  \frac{1}{|I_F|} \sum\limits_{\vx_i \in I_F}
    w_F( f/\sigma_F - I_F(\vx_i)/\sigma_F ) \\
     &\relphantom{=}\times w_M( m/\sigma_M - I_M(\vT_{\vmu}(\vx_i))/\sigma_M ),
\end{split}
\end{align}
...
suitable for mono- and multi-modality registration. In our
experience it gives results very similar to MI. The KS measure is
specifically meant to register binary images (segmentations). It
measures the ``overlap'' between the segmentations.
