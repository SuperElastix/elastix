...
by a uniform grid of control points. This grid is overlaid on the
fixed image and defined by the spacing between the grid nodes. [show
figure with a grid] The spacing defines how dense the grid is, or
what the locality is of the transformation you can model. For each
resolution level you can define a different grid spacing. This is
what we call multi-grid. In general, we recommend to start with a
coarse B-spline grid, i.e. a more global transformation. This way
first the larger structures are matched, for the same reason as why
you should start with a rigid or affine transformation. In later
resolutions you can stepwise refine the transformation; the idea is
that you then match smaller structures, up to the final

\item[On the step-size:]
An important parameter to tune is, as said before, the gain factor.
,combo with number of iterations
\end{description}


\section{Overview of all parameters}

see the elastix website:
http://www.isi.uu.nl/Elastix/doxygen/parameter.html


\section{Trouble shooting}

initial alignment not good, rotation parameter, large steps
