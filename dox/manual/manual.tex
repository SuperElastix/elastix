...
the authors' own experiences. Chapter~\ref{chp:develop} provides more
details about the source code and explains how to create extensions
of \elastix.
...
(model) of the transformation. The original optimisation problem
becomes thus:
\begin{align}
...
\item[Normalized Mutual Information (NMI):] (\texttt{NormalizedMutualInformationMetric}) //add description
\cite{Stu99:NMI}

\item[Kappa Statistic (KS):] (\texttt{AdvancedKappaStatisticMetric}) //add description
...
fixed, deformed moving

\begin{itemize}
\item Compare images side by side in a viewer. deformed should
be similar to fixed. Also deformed image should have same texture as
moving image, so no sudden blurred areas in deformed.

\item Compute a difference image if monomodal

\item Compute distance between manually clicked corresponding points

\item Compute overlap of anatomical structures

\item check deformation field by determinant of the Jacobian <1,1,>1

\item show exact metric value, and plot it. For example with Matlab
(give Matlab code)
\end{itemize}
...
The \texttt{RandomCoordinate} works well in conjunction with the
\texttt{StandardGradientDescent} optimiser, which is the recommended
optimisation routine. This optimisation method can be used with a
small amount of samples, randomly selected in every iteration, see
Section \ref{sec:comp:optimiser}, which significantly decreases
registration time. Set the \texttt{NumberOfSpatialSamples} to 3000.
Don't go lower than 2000. The \texttt{RandomCoordinate} sampler
avoids what is known as the grid-effect [citations].
...
We recommend a third order B-spline interpolator for generating the
resulting deformed moving image:
\begin{quote}
...
down-sampling in case a FixedRecursiveImagePyramid is used), in each
direction $x,y,z$ and for each resolution level. It can be specified
as follows:
\begin{quote}
\texttt{(NumberOfResolutions 4)} \\
...
when you use masks you only perform registration on the part of the
image that is within the masks.
...
How does the use of masks influence your results
