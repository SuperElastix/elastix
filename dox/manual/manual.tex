...
information on how to use it in \elastix.
...
Generally speaking, you first want to perform a rigid transformation,
before doing a nonrigid one. When doing a registration one carefully
has to choose the several components, as specified in Chapter
\ref{chp:components}. In Table \ref{table:recom:components} some
recommendations are given.

See http://www.isi.uu.nl/Elastix/doxygen/modules.html for an overview
...
FixedImagePyramid & \texttt{FixedRecursiveImagePyramid}: this pyramid
will smooth and down-sample the fixed image for you, according to the
\texttt{FixedImageSchedule} you provide. If you only set the
\texttt{NumberOfResolutions}, a default schedule will be used that
down-samples the fixed image by 2. \\
MovingImagePyramid & See the FixedImagePyramid. \\
Metric    & \texttt{AdvancedMattesMutualInformation}: usually works
well, both for mono- and multi-modal images. \\
